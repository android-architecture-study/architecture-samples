...
class StatisticsViewModel(
            ...
            viewModelScope.launch {
                getTasksUseCase().let { result ->
                    if (result is Success) {
                        _error.value = false
                        computeStats(taskListMapper.toModel(result.data))
                    } else {
                        _error.value = true
                        activeTasks = 0
                        completedTasks = 0
     ...
     */
    private fun computeStats(taskModels: List<TaskModel>?) {
        getActiveAndCompletedStats(taskModels).let {
            _activeTasksPercent.value = it.activeTasksPercent
            _completedTasksPercent.value = it.completedTasksPercent
        }
        _empty.value = taskModels.isNullOrEmpty()
